-- ═══════════════════════════════════════════════════════════════════════════
-- Tabela de Bios de Prospecção — avos_sessao.prospect_bios
-- ═══════════════════════════════════════════════════════════════════════════
-- Armazena dados pré-coletados de leads para o agente fazer
-- Apreciação Sincera personalizada antes da abordagem.
--
-- Popule esta tabela via scraping/n8n antes de ativar o agente.
-- ═══════════════════════════════════════════════════════════════════════════

CREATE TABLE IF NOT EXISTS avos_sessao.prospect_bios (
...
CREATE INDEX IF NOT EXISTS idx_prospect_bios_sessao_username
  ON avos_sessao.prospect_bios (instagram_username);

-- View acessível pelo Table Editor (schema public)
CREATE OR REPLACE VIEW public.avos_sessao_prospect_bios AS
  SELECT * FROM avos_sessao.prospect_bios ORDER BY created_at DESC;

-- Exemplo de insert (remover/adaptar):
INSERT INTO avos_sessao.prospect_bios (instagram_username, nome, bio, nicho, post_destaque, post_url)
VALUES
