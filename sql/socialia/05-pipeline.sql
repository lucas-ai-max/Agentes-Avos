-- ═══════════════════════════════════════════════════════════════════════════
-- Pipeline de Prospecção SocialIA — Agent Avos
-- ═══════════════════════════════════════════════════════════════════════════
-- Rode este SQL no Supabase SQL Editor.
-- Cria a tabela avos.pipeline_stages com as 11 stages da jornada.
-- ═══════════════════════════════════════════════════════════════════════════

CREATE TABLE IF NOT EXISTS avos.pipeline_stages (
...
TRUNCATE TABLE avos.pipeline_stages RESTART IDENTITY;

INSERT INTO avos.pipeline_stages (stage_key, label, crm_status, ordem, description, next_step) VALUES
...
CREATE OR REPLACE VIEW public.avos_pipeline_stages AS
  SELECT * FROM avos.pipeline_stages ORDER BY ordem;

-- Conferir:
-- SELECT * FROM avos_pipeline_stages;
